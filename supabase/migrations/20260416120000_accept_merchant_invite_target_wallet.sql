-- Align accept_merchant_invite with invites that pin a specific wallet (target_wallet_address).
...
BEGIN
  ...
  SELECT
    i.id,
    lower(i.merchant_address),
    i.branch_id,
    i.role,
    lower(trim(i.target_wallet_address))
  INTO v_inv_id, v_merchant, v_branch, v_role, v_target
  ...
  IF v_target IS NOT NULL AND v_target <> '' AND v_target <> v_wallet THEN
    RETURN jsonb_build_object('ok', false, 'error', 'invite_not_found');
  END IF;
...
COMMENT ON FUNCTION public.accept_merchant_invite(text) IS
  'Redeem a pending merchant team invite; uses profiles.wallet_address. Honors target_wallet_address when set.';
