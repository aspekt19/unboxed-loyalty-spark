...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return jsonResponse({
      ...
      "POST /mint": "Record mint intent; returns a fee-first calls[] bundle (protocol_fee then recipient_mint). The fee is minted in your own loyalty tokens, not USDC — settle it via POST /mint/confirm",
      "POST /mint/confirm": "Settle a mint fee obligation { obligation_id, fee_tx_hash }. 5+ obligations unpaid for over 60 minutes block further mints with HTTP 402",
