    ...
    const { error } = await supabase
      ...
      .insert(
        rewardsToMigrate.map(r => ({
          id: r.id,
          token_address: r.tokenAddress.toLowerCase(),
          merchant_address: r.merchantAddress.toLowerCase(),
          name: r.name,
    ...
    const { error } = await supabase
      ...
      .insert(
        vouchersToMigrate.map(v => ({
          ...
          token_address: v.tokenAddress.toLowerCase(),
          token_symbol: v.tokenSymbol,
          customer_address: v.customerAddress.toLowerCase(),
          merchant_address: v.merchantAddress.toLowerCase(),
          status: v.status,
          cost: v.cost,
