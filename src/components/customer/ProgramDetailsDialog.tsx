...
export function ProgramDetailsDialog({ open, onOpenChange, tokenAddress, balance }: ProgramDetailsDialogProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        console.error('[ProgramDetailsDialog] load failed', err);
      } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] p-0 gap-0 overflow-hidden">
        <DialogHeader className="p-6 pb-4 border-b bg-gradient-to-br from-primary/10 via-card to-uds-lavender-light/40">
          <div className="flex items-start gap-4">
            {merchant?.logo_url ? (
              <img
                src={merchant.logo_url}
                alt={merchant.business_name}
                className="h-14 w-14 rounded-xl object-cover border-2 border-border flex-shrink-0"
              />
            ) : (
              <div className="h-14 w-14 rounded-xl bg-gradient-to-br from-primary to-uds-orange flex items-center justify-center flex-shrink-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <DialogDescription className="flex items-center gap-2 flex-wrap mt-1">
                <span className="font-mono text-xs">{program?.symbol}</span>
                {program && (
              ...
              <>
                  ...
                  <CardContent className="p-4 flex items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-3xl font-bold">
                        ...
                        <span className="text-sm text-muted-foreground font-normal">{program.symbol}</span>
                      </p>
                      {currentTier && (
                ...
                <section>
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    Formula: <span className="font-mono">amount × ({Number(program.cashback_rate)}% ÷ 100) × {Number(program.points_per_dollar)}</span>
                  </p>
                </section>
                              ...
                              <div className="flex items-center gap-3 min-w-0">
                                ...
                                <div className="min-w-0">
                                  <p className="text-sm font-medium truncate">
                                    {t.tier_name}
                                    {isCurrent && (
                                      <Badge variant="default" className="ml-2 text-[9px]">Current</Badge>
                                    )}
                                  </p>
                            ...
                            <CardContent className="p-3 flex items-center justify-between gap-3">
                              ...
                              <div className="text-right flex-shrink-0">
                                <p className="text-sm font-bold">
                                  {Number(r.cost)} <span className="text-xs text-muted-foreground font-normal">{program.symbol}</span>
                                </p>
                                <Badge
