# B20 Native Spec (Beryl) for Loyalty Tokens

Understand what a Loyal Spark **B20** loyalty token is at the chain level, so an agent can reason about roles, policies, pausing, memos and permits without guessing.
...
Loyal Spark currently emits standard `mint` / `transfer` calldata (no memo). The public API does not currently accept a `memo` field. Agents must not claim memo attribution was recorded unless the returned calldata explicitly identifies a memo variant.

## 6. Other spec facts worth knowing

- **Supply cap**: `type(uint128).max` means "no cap". Loyal Spark leaves loyalty programs uncapped.
- **ERC-2612 permit**: EIP-712 domain version is `"1"`. **ERC-1271 signatures are NOT accepted** for permit — smart accounts must call `approve` in a normal transaction.
- B20 is fully ERC-20 compatible at the operation interface.

## 7. Network facts

- Base Mainnet chainId **8453**, RPC `https://mainnet.base.org`.
- **Flashblocks**: ~200 ms preconfirmations. Poll receipts at ~250–500 ms when waiting on Base.
- **Vibenet** is Base's demo network — Loyal Spark does **not** deploy there; use Base Sepolia (84532) for testing.

## Related

- Web skill copy: https://loyalspark.online/.well-known/skills/14-b20-native-spec.md
- Create program: https://loyalspark.online/.well-known/skills/01-create-loyalty-program.md
- Payment scenarios: [payment-scenarios.md](./payment-scenarios.md)
