...
- **Merchant REST** (`lsk_`, `/agent-api`): 27 authenticated routes + 1 public GET `/vouchers/status` (includes `POST /workflow/generate-program-defaults`, `GET /workflow/program-status`).
- **Recipient REST** (`rwk_`, `/recipient-api`): 14 routes (includes `GET /workflow/reward-status`, `POST /workflow/prepare-reward-redemption`).
- **Merchant MCP** (`/loyalty-mcp`): 38 tools (34 core + 4 Bazaar side-car). Workflow planners: `generate_program_defaults`, `get_program_workflow_status`. Registry: `src/constants/mcpToolNames.ts`.
- **Recipient MCP** (`/recipient-loyalty-mcp`): 20 tools (16 core + 4 Bazaar side-car). Workflow planners: `get_reward_workflow_status`, `prepare_reward_redemption`. Registry: `src/constants/recipientMcpToolNames.ts`.
