...
price_card(Inches(6.85), "🤖 AI-агенты (API + MCP)", [
    ("Free", "$0", "200 вызовов · 1 агент · 1,000 токенов mint/мес · 1.25% mint fee"),
    ("Pro", "$49/мес", "10 000 вызовов · 0.50%"),
    ("Enterprise", "$129/мес", "∞ вызовов · 0.25%"),
])
textbox(s, Inches(0.9), Inches(6.25), Inches(11.5), Inches(0.7),
        [[("Pay-per-call (x402 / MPP): от ~$0.001 за чтение, ~$0.005–0.05 за запись. Денежная выручка — подписки и оплата за вызов (предоплата в USDC). Mint fee списывается в баллах мерчанта — протокольный учёт, не ARR.", 12, MUTED, False)]], line_spacing=1.1)
footer(s, 8)
...
moat = [
    ...
    ("💰", "Модель «два счёта»", "SaaS бизнеса + API агентов + x402 оплата за вызов — диверсификация в USDC."),
    ...
    ("🪙", "Токен у сообщества", "65% $LOYAL — участникам, ликвидность залочена навсегда."),
]
...
subtitle(s, "Сценарий на базе подписок в USDC и оплаты за вызов (x402 / MPP) — модель работает уже сегодня.")
...
textbox(s, Inches(0.9), Inches(6.5), Inches(11.5), Inches(0.5),
        [[("Прогноз иллюстративный. Выручка считается по платящим подписчикам и оплате за вызов, часть бизнесов остаётся на бесплатном тарифе. $LOYAL в прогноз не входит.", 11.5, MUTED, False)]])
footer(s, 13)

# ============ SLIDE 14 — TOKENOMICS ============
s = prs.slides.add_slide(BLANK); grad_bg(s)
kicker(s, "13 · Токеномика")
title(s, "LOYAL — управление и ликвидность")
# left card: allocation
card(s, Inches(0.9), Inches(2.45), Inches(5.6), Inches(4.1))
textbox(s, Inches(1.15), Inches(2.65), Inches(5.1), Inches(0.5), [[("Распределение · 1 000 000 000 LOYAL", 15, WHITE, True)]])
alloc = [("Backers", "65.0%", 0.65), ("Ликвидность (навсегда)", "15.0%", 0.15),
         ("Treasury (governance)", "10.0%", 0.10), ("Founder (time-lock)", "7.5%", 0.075),
         ("Экосистема / Airdrop", "2.5%", 0.025)]
yy = Inches(3.25)
...
textbox(s, Inches(7.1), Inches(2.65), Inches(5.1), Inches(0.5), [[("Что это даёт", 15, WHITE, True)]])
util = [("🤝", "65% — участникам.", "Основатель ограничен 7.5% на уровне контракта, поднять нельзя."),
        ("🔒", "Ликвидность не вывести.", "15% залочены в Aerodrome навсегда — вектор rug закрыт."),
        ("🗳️", "Governance без стейкинга.", "Treasury разблокируется только голосованием держателей.")]
yy = Inches(3.3)
for ic, h, b in util:
...
textbox(s, Inches(0.9), Inches(6.62), Inches(11.5), Inches(0.35),
        [[("Запуск на Vibestarter (Base): аллокация зашита в контракт при деплое. Выручка платформы — подписки и оплата за вызов в USDC, $LOYAL для оплаты сервиса не используется.", 11.5, MUTED, False)]])
footer(s, 14)
...
ms = [
    ...
    ("💵", "Регулярная выручка", "Подписки и оплата за вызов в USDC."),
