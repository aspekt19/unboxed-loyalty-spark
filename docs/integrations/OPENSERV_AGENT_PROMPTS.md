...
> Full catalogue: **28** tools — `supabase/functions/loyalty-mcp/index.ts`. Subsets below are the ones most relevant to this workflow.
...
> Full catalogue: **28** tools — `supabase/functions/loyalty-mcp/index.ts`.
...
- ✅ agent.json / MCP tool list matches `loyalty-mcp/index.ts` (**28** tools including reporting and admin tools)
