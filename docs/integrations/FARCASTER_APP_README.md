...
### For AI Agents
- **REST API**: 28 authenticated routes + **GET `/vouchers/status`** (public); see root `README.md` or `public/llms-full.txt`
- **MCP Server**: **39** merchant tools + **20** recipient tools — Streamable HTTP; same surface as REST for LLM clients
...
| Function | Purpose |
|----------|---------|
| `agent-api` | REST API for AI agents (28 authenticated routes + public GET `/vouchers/status`) |
| `agent-api-key` | API key generation |
| `agent-wallet` | CDP MPC wallet management |
| `loyalty-mcp` | MCP Server for LLMs (39 merchant tools; recipient MCP has 20) |
